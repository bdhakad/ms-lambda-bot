...
const helloCard = require("./adaptiveCards/hello.json");
const cardTools = require("@microsoft/adaptivecards-tools");
var {LexRuntimeV2Client, RecognizeTextCommand } = require("@aws-sdk/client-lex-runtime-v2");
...
class TeamsBot extends TeamsActivityHandler {
  constructor() {
    ...
    this.onMessage(async (context, next) => {
      ...
      if(txt.toLowerCase() ==="hi" || txt.toLowerCase() ==="hello" || txt.toLowerCase() ==="hey"){
        txt = "hello";
      }
      // Trigger command by IM text
      switch (txt) {
        case "welcome": {
          console.log("welcome : Running with Message Activity.");
          const card = cardTools.AdaptiveCards.declareWithoutData(rawWelcomeCard).render();
          await context.sendActivity({ attachments: [CardFactory.adaptiveCard(card)] });
          break;
        }
        case "learn": {
          console.log("learn : Running with Message Activity.");
          this.likeCountObj.likeCount = 0;
        ...
        case "hello": {
          console.log("echo : Running with Message Activity.");
          const card = cardTools.AdaptiveCards.declareWithoutData(helloCard).render();
          await context.sendActivity({ attachments: [CardFactory.adaptiveCard(card)] });
          break;
        }
        /**
        ...
        default: {
          ...
          console.log("input: " + context.activity.text);
          try {
            const data = await client.send(command);
            // console.log("Lex Response data.messages: ", JSON.stringify(data.messages));
          if(!data.messages.length){
              await context.sendActivity(`I didn't understand that.`);
          }else{
    ...
    this.onMembersAdded(async (context, next) => {
      ...
      console.log("Running with MembersAdded event.");
